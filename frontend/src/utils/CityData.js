...
const itemBase = {
  aluminum_cans: {
    ...
    description:
      "Lightweight, recyclable cans used for drinks and other products.",
    category: "Cans",
  },
  aluminum_foil: {
    name: "Aluminum Foil",
  ...
  plastic_bottle: {
    ...
    description:
      "Recyclable plastic containers used for drinks and other products.",
    category: "Plastic",
  },
  cans: {
    name: "Cans",
  ...
  cartons: {
    ...
    description:
      "Containers for drinks and other products, often made from paper.",
    category: "Paper",
  },
  detergent_bottle: {
...
const generateCityData = (cityName, excludedItems = []) => {
  const allItems = _.values(itemBase);

  // array where each item includes a canRecycle property
  const cityItems = _.map(allItems, (item) => ({
    ...item,
...
export const categoryData = Array.from(new Set(generateCategories()));
